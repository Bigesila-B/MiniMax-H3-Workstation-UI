...
- 🧹 **生成后自动清理显存**：「高级参数」内置滑动开关（默认开启），每个视频合成后执行一次 `easy cleanGpuUsed` 清理 GPU 缓存；全能参考模式按需自动注入清理节点。
...
- 需要以下 ComfyUI 自定义节点：
  ...
  - **ComfyUI-Easy-Use**（显存清理节点 `easy cleanGpuUsed`，普通模式工作流已内置使用；「生成后自动清理显存」开关在全能参考模式下动态注入时也依赖它）
